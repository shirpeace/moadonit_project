...
public class ActivityController extends HttpServlet implements Serializable {
	...
	private boolean deletePupilActivity(HttpServletRequest req,
		...
		pa = (PupilActivity) DAOUtil.getObjectFromJson(
				req.getParameter("pupilActivity"), pa.getClass());
	...
	private boolean editPupilInCourse(HttpServletRequest req,
		...
		if (req.getSession().getAttribute("userid") != null) {
			JSONObject user = (JSONObject) req.getSession().getAttribute(
					"userid");
			User u = new User();
	...
	@SuppressWarnings("unchecked")
	private boolean insertPupilActivity(HttpServletRequest req,
			HttpServletResponse resp) {
		...
		try {
			...
			if (req.getSession().getAttribute("userid") != null) {
				JSONObject user = (JSONObject) req.getSession().getAttribute(
						"userid");
				User u = new User();
				u = (User) DAOUtil.getObjectFromJson(user.toString(),
						u.getClass());

				pa = (PupilActivity) DAOUtil.getObjectFromJson(str,
						PupilActivity.class);
				pa.setTblUser(u);
	...
	@SuppressWarnings("unchecked")
	private Boolean updateCourse(HttpServletRequest req,
		...
		java.sql.Time sTime = new Time(Integer.parseInt(start[0]),
				Integer.parseInt(start[1]), 00);

		String endTime = req.getParameter("endTime");
		String[] end = endTime.split(":");
		@SuppressWarnings("deprecation")
		java.sql.Time eTime = new Time(Integer.parseInt(end[0]),
				Integer.parseInt(end[1]), 00);
		/*
		 * String endTime = req.getParameter("endTime"); SimpleDateFormat format
		 * = new SimpleDateFormat("HH:mm:ss"); Date date =
		 * format.parse(startTime);
		 */
		// Time t = new Time();

		act = (Activity) DAOUtil
				.getObjectFromJson(activityData, Activity.class);
	...
	private List<Activity> searchCourses(HttpServletRequest req,
		...
		isRegular = (isRegular != null && isRegular.trim().equals("")) ? null
				: isRegular;

		String staffName = req.getParameter("staffName");
		staffName = (staffName != null && staffName.trim().equals("")) ? null
				: staffName;

		// req.getParameter("regularOrPrivate").trim().equals("") ? null :
		// req.getParameter("regularOrPrivate");
		...
		result = this.actDOA.searchCoursesByParam(
				...
				staffName,
				req.getParameter("pricePerMonth") != null ? Float
						.parseFloat(req.getParameter("pricePerMonth")) : 0,
				req.getParameter("extraPrice") != null ? Float.parseFloat(req
						.getParameter("extraPrice")) : 0, isRegular, 0 // no
																		// need
																		// for
																		// category
	...
	protected JSONObject getJsonActivities(Activity act) {
		...
		// /activityNum, activityType, activityName, weekDay, startTime,
		// endTime, schoolYear, responsibleStaff, activityNum, pricePerMonth,
		// extraPrice, regularOrPrivate, category

		JSONObject obj = new JSONObject();

		obj.put("activityNum", act.getActivityNum());
		obj.put("activityType", act.getTblActivityGroup().getTblActivityType()
				.getTypeID());
		...
		String fn = act.getTblStaff().getFirstName() != null ? act
				.getTblStaff().getFirstName() : "";
		String ln = act.getTblStaff().getLastName() != null ? act.getTblStaff()
				.getLastName() : "";
	...
	protected void getJsonCourses(JSONArray registrationData) {
		...
		for (Activity act : this.listAct) {
			...
			obj.put("regularOrPrivate", act.getTblCourse()
					.getRegularOrPrivate());
